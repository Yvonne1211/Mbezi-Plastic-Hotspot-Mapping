...
Map.addLayer(table, {color:'orange'}, 'p2018');
Map.centerObject(table, 15);
// 1️⃣ Cloud masking function for Landsat 8
function maskClouds(image) {
  var qa = image.select('QA_PIXEL');
  var cloud = qa.bitwiseAnd(1 << 3);
  var cirrus = qa.bitwiseAnd(1 << 4);
  return image.updateMask(cloud.not().and(cirrus.not()));
}
...
// 3️⃣ Load Landsat 8 & median composite
...
var medianComposite = landsat8.reduce(ee.Reducer.median());
...
// Shadow mask using brightness (Red + NIR + SWIR1)
var brightness = filledComposite.select('SR_B4')
                  .add(filledComposite.select('SR_B5'))
                  .add(filledComposite.select('SR_B6'));
var shadowMask = brightness.gte(0.05);
var correctedComposite = filledComposite.updateMask(shadowMask);

var annualComposite = applyScaleFactors(correctedComposite).clip(studyArea);

// RGB visualization (True color)
Map.addLayer(annualComposite, {bands:['SR_B4','SR_B3','SR_B2'], min:0, max:0.3}, 'RGB Composite');
...
var stats = image_stack.select(['PI2', 'BSI']).reduceRegion({
  ...
  geometry: studyArea,
  scale: 30,              // ⚠️ IMPORTANT for Landsat
  maxPixels: 1e13,
  bestEffort: true
});

// 7️⃣ Print AFTER computation
print('Percentiles (PI2 & BSI):', stats);
...
var bandsForClassification = [
  'SR_B2','SR_B3','SR_B4',
  'SR_B5','SR_B6','SR_B7','BSI','MNDWI','PSI',
  'NDVI','MNDBI','PI2_corrected','SWIRdiff'
...
var palette = ['006400','1E3A8A','FF1A1A','FF8C00','A0522D']; // Vegetation, Waterbody, Built-up, Debris, Bareground
...
var varImportanceChart = ui.Chart.feature.byProperty(importance)
  ...
  .setOptions({
    title: 'Random Forest Variable Importance 2018',
...
// 15️⃣ Export classified map
//Export.image.toDrive({
  //image: Classified,
// description: 'Landsat8_RF_2018',
 // region: studyArea,
 // scale: 30,
